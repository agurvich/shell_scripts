...
do
    ...
    echo "#SBATCH -J ${gal_str}_${snapnum}_${uniform}          # job name" >> temp.sh
    echo "cd $HOME/starformation/scripts" >> temp.sh
    for patch in "${patches[@]}"
    do
        echo "python makeFullHistory.py --snaplow=$snapnum --snaphigh=$snapnum --savename=$fname --mps=1 --patch=${patch} --uniform=${uniform}" >> temp.sh
    done
